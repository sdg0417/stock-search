...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        # [F-1] path parameter 파싱: /api/quote/{symbol} 경로에서 마지막 세그먼트 추출
        parsed = urllib.parse.urlparse(self.path)
        symbol = urllib.parse.unquote(parsed.path.split('/')[-1])
        ...
        # [F-8] 두 호스트 모두 실패 시 stderr 로깅
        print(f"quote error: {last_err}", file=sys.stderr)
